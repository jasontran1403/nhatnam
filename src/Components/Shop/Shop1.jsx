...
const Shop1 = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.code === STATUS_CODE.SUCCESS) {
        ...
        setError(response.message || t("content.error_load"));
      }
    } catch (err) {
      console.error("Error fetching products:", err);
      setError(err.message || t("content.error_load"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesPrice =
      product.defaultPrice >= priceRange.min && product.defaultPrice <= priceRange.max;
  ...
  const fromResult = totalItems === 0 ? 0 : currentPage * pageSize + 1;
  const toResult = Math.min((currentPage + 1) * pageSize, totalItems);
            ...
            <div className="col-xl-3 col-lg-4 order-2 order-md-1 wow fadeInUp" data-wow-delay=".3s">
              <div className="main-sidebar">
                ...
                <div className="single-sidebar-widget">
                  <h5 className="widget-title">{t("sidebar.search_title")}</h5>
                ...
                {/* Categories */}
                <div className="single-sidebar-widget">
                  <h5 className="widget-title">{t("sidebar.categories_title")}</h5>
                ...
                {/* Price Range */}
                <div className="single-sidebar-widget">
                  <h5 className="widget-title">{t("sidebar.filter_price_title")}</h5>
                    ...
                    <div className="range-items">
                      <div className="price-input">
                        <div className="price-wrapper d-flex align-items-center gap-1">
                          <div className="field">
                            <span>{t("sidebar.price_label")}</span>
                          </div>
                          <div className="field">
                            <input
                              type="number"
                          ...
                          <a href="#" className="filter-btn mt-2 me-3" onClick={(e) => e.preventDefault()}>
                            {t("sidebar.filter_btn")}
                          </a>
                        </div>
                      </div>
                    </div>
                ...
                <div className="row g-sm-0 gy-20 justify-content-between align-items-center">
                  <div className="col-md">
                    <p className="woocommerce-result-count">
                      {t("sortbar.showing", { from: fromResult, to: toResult, total: totalItems })}
                    </p>
                  </div>
                    ...
                    <form className="woocommerce-ordering" method="get">
                      <select name="orderby" className="single-select" aria-label="Shop order" onChange={handleSortChange}>
                        <option value="menu_order" selected="selected">
                          {t("sortbar.default_sort")}
                        </option>
                        <option value="popularity">{t("sortbar.sort_popularity")}</option>
                        <option value="date">{t("sortbar.sort_latest")}</option>
                        <option value="price">{t("sortbar.sort_price_asc")}</option>
                        <option value="price-desc">{t("sortbar.sort_price_desc")}</option>
                      </select>
                    </form>
                  </div>
              ...
              <div className="tab-content" id="pills-tabContent">
                ...
                >
                  {loading ? (
                    ...
                    <div className="alert alert-info text-center py-5">{t("content.not_found")}</div>
                  ) : (
                    <div className="dishes-card-wrap style2">
                      {filteredProducts.map((product) => (
                        <ShopCard
                          ...
                          title={product.name}
                          content={product.description || t("content.default_desc")}
                ...
                <div
                  ...
                  tabIndex="0"
                >
                  <p className="text-center py-5">{t("content.list_mode_dev")}</p>
                </div>
              </div>
