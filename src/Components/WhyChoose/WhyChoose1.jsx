...
const WhyChoose1 = () => {
  ...
  const handleClick = () => {
    setVideoSrc("https://player.cloudinary.com/embed/?cloud_name=dflodky4j&public_id=c737bc8c-a6b4-46ec-8c7c-88489d0f9336_y7cfr5");
    setToggle(true);
  };
  const handleClose = () => { setVideoSrc(""); setToggle(false); };

  return (
    <section ref={sectionRef} className="whychoose-modern fix" style={{ position: 'relative', overflow: 'hidden', background: '#0C0D0F' }}>
      ...
      <style>{`
        ...
        .wc-video-wrap img.thumb {
          width: 100%;
          display: block;
          object-fit: cover;
          transition: transform .8s cubic-bezier(.25,.46,.45,.94);
        }
        .wc-video-wrap:hover img.thumb { transform: scale(1.04); }
        ...
        .wc-play-btn:hover .wc-play-ring {
          transform: scale(1.1);
          background: rgba(230,120,40,.95);
        }
        .wc-play-ring::before {
          content: '';
          position: absolute;
        ...
        .wc-eyebrow-dot {
          ...
          border-radius: 50%;
          background: #E8640E;
          box-shadow: 0 0 8px rgba(232,100,14,.8);
          animation: wc-dot-blink 2s ease infinite;
        }
        ...
        .wc-feature-card::before {
          ...
          position: absolute;
          inset: 0;
          background: linear-gradient(135deg, rgba(201,100,30,.08), transparent);
          opacity: 0;
          transition: opacity .35s;
        }
        .wc-feature-card:hover {
          border-color: rgba(201,100,30,.35);
          transform: translateX(6px);
          background: rgba(201,100,30,.06);
        }
        ...
        .wc-feature-icon {
          ...
          border-radius: 12px;
          background: rgba(201,100,30,.15);
          border: 1px solid rgba(201,100,30,.25);
          display: flex;
          align-items: center;
          justify-content: center;
          flex-shrink: 0;
        ...
        .wc-cta {
          ...
          letter-spacing: .3px;
          box-shadow: 0 8px 28px rgba(232,100,14,.4);
          transition: all .3s;
          position: relative;
          overflow: hidden;
        }
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        <div className="row gx-5 gy-5 align-items-center">
          ...
          <div className="col-lg-6">
            <div className={`wc-fade-left ${visible ? 'visible' : ''}`}>
              <div className="wc-video-wrap">
                <img className="thumb" src="/assets/img/history/historyThumb1_1.jpg" alt="thumb" />

                <button type="button" className="wc-play-btn" onClick={handleClick} aria-label="Xem video">
                  <div className="wc-play-ring">
                    <div className="wc-play-icon" />
                  </div>
                </button>
              </div>
            ...
            <div className={`wc-fade ${visible ? 'visible' : ''}`} style={{ transitionDelay: '.15s' }}>
              ...
              <div className="wc-features">
                {items.slice(0, 2).map((item, i) => (
                  <div key={i} className={`wc-feature-card wc-fade ${visible ? 'visible' : ''}`}
      ...
      <VideoModal isTrue={toggle} videoSrc={videoSrc} handelClose={handleClose} />
    </section>
  );
};
